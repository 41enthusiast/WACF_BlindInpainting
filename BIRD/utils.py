...
def DDIM_efficient_feed_forward(latent, model, ddim_scheduler):
    for i, t in enumerate(tqdm1(ddim_scheduler.timesteps)):# multiple tqdms defined, to resolve at the end
        t1 = (torch.ones(1) * t).to(latent.device)
        with torch.no_grad():
            if i == 0:
        ...
        if i == 0:
            x_t = ddim_scheduler.step(noise_pred, t, latent, return_dict=True, use_clipped_model_output=True)
        else:
            x_t = ddim_scheduler.step(noise_pred, t, x_t, return_dict=True, use_clipped_model_output=True)
    return x_t
