...
from utils import get_timestep_embedding, inverse_data_transform, save_image, nonlinearity
...
class DiffusiveRestoration:
    def __init__(self, diffusion, resume, image_folder, config):
        ...
        if os.path.isfile(resume):
            self.diffusion.load_ddm_ckpt(resume, ema=True)
            self.diffusion.model.eval()
        else:
            print('Pre-trained diffusion model path is missing!')

    def restore(self, val_loader, validation='snow', r=None):
        image_folder = os.path.join(self.image_folder, self.config.data.dataset, validation)
        with torch.no_grad():
